...
void dipole(double *vx,double *vy,double *vz,double *psi,double *mux,double *muy,double *muz,double *eval,long_st ist,par_st par)
{
  ...
  pf = fopen("OS0.dat" , "w"); 
  pf1 = fopen("ux0.dat", "w"); pf2 = fopen("uy0.dat", "w"); pf3 = fopen("uz0.dat", "w");
  fprintf(pf, "i a  sqrt(|u|^2)    Ea-Eb     4/3*E*|u|^2     ux         uy        uz\n"); //Daniel C. addition
  ...
  for (i = 0; i < ist.totalhomo; i++, fprintf(pf1,"\n"),fprintf(pf2,"\n"),fprintf(pf3,"\n")){
    for (a = ist.nlumo; a < ist.nlumo+ist.totallumo; a++) {
      ...
      fprintf(pf,"%ld %ld %.8f %.12f %.8f %.8f %.8f %.8f\n", i, a, sqrt(os), ev, (4.0/3.0)*ev*os,
	       mux[i*ist.totallumo+(a-ist.nlumo)],
	       muy[i*ist.totallumo+(a-ist.nlumo)],
	       muz[i*ist.totallumo+(a-ist.nlumo)]);
    }
  }
  ...
  double *eval, fftw_plan_loc *planfw,fftw_plan_loc *planbw,fftw_complex *fftwpsi, long_st ist, par_st par)
{
  FILE *pf1, *pf; 
  long a, i, jx, jy, jz, jgrid, jyz; 
  double sumX, sumY, sumZ, Px, Py, Pz;
  ...
  pf = fopen("M0.dat" , "w"); 
  pf1 = fopen("p0.dat", "w"); 
  //fprintf(pf, "i a  sqrt(|m|^2)    Ea-Eb     16/3*E*|u|^2     mx         my        mz\n"); //Daniel C. addition
  ...
  for (i = 0; i < ist.totalhomo; i++, fprintf(pf1,"\n")) {
    ...
    for (a = ist.nlumo; a < ist.nlumo+ist.totallumo; a++) {
      ...
      for (jz = 0; jz < ist.nz; jz++) {
        ...
        for (jy = 0; jy < ist.ny; jy++) {
          ...
          for (jx = 0; jx < ist.nx; jx++) {
            x = vx[jx];
            jgrid = jyz + jx;
            tmp = psi[a*ist.ngrid+jgrid] * par.dv;
      ...
      mx[i*ist.totallumo+(a-ist.nlumo)] =  sumX * 0.5;
      my[i*ist.totallumo+(a-ist.nlumo)] =  sumY * 0.5;
      mz[i*ist.totallumo+(a-ist.nlumo)] =  sumZ * 0.5;
      fprintf(pf1,"%ld %ld %.8f %.8f %.8f\n", i, a, Px, Py, Pz);

      ms=(sqr(mx[i*ist.totallumo+(a-ist.nlumo)]) + sqr(my[i*ist.totallumo+(a-ist.nlumo)]) + sqr(mz[i*ist.totallumo+(a-ist.nlumo)]));
      ev = eval[a] - eval[i];
      fprintf(pf,"%ld %ld %.8f %.12f %.8f %.8f %.8f %.8f\n", i, a, sqrt(ms), ev, (16.0/3.0)*ev*ms,
         mx[i*ist.totallumo+(a-ist.nlumo)],
         my[i*ist.totallumo+(a-ist.nlumo)],
         mz[i*ist.totallumo+(a-ist.nlumo)]);
    }
  }
