...
void read_pot(double *vr,double *pot,long *npot,double *dr,atm_st *atm,long n,long ntype)
{
  ...
  pf = NULL;
  pf = fopen("pot_chiral_ligand.par","r");
  if(pf!=NULL){
    fscanf(pf,"%lg",&a[10]); // P3 a
    fscanf(pf,"%lg",&b[10]); // P3 b
    fscanf(pf,"%lg",&a[18]); // P5 a
    fscanf(pf,"%lg",&b[18]); // P5 b
    fscanf(pf,"%lg",&a[19]); // P6 a
    fscanf(pf,"%lg",&b[19]); // P6 b
    fclose(pf);
  }
  ...
  /*
      ...
      Se1 = 15
      P = 16
      PC5 = 16 // use for chiral ligand, read in from chiral_ligand_pot
      PC6 = 17 // use for chiral ligand, read in from chiral_ligand_pot
...
long assign_atom_number(char atyp[3])
{
  ...
  else if ((atyp[0] == 'S') && (atyp[1] == 'e') && (atyp[2] == '1')) return(17);
  else if ((atyp[0] == 'P') && (atyp[1] == 'C') && (atyp[2] == '5'))  return(18);
  else if ((atyp[0] == 'P') && (atyp[1] == 'C') && (atyp[2] == '6'))  return(19);
  else {
...
void assign_atom_type(char *atyp,long j)
{
  ...
  else if (j == 17) {atyp[0] = 'S'; atyp[1] = 'e'; atyp[2] = '1';}
  else if (j == 18) {atyp[0] = 'P'; atyp[1] = 'C'; atyp[2] = '5';}
  else if (j == 19) {atyp[0] = 'P'; atyp[1] = 'C'; atyp[2] = '6';}
  return;
}
