...
int main(int argc, char *argv[]) {
    ...
    par_st par;
    long_st ist;
    printf("\nRUNNING PROGRAM: SOLVE BETHE-SALPETHER\n\n");
    /*************************************************************************/
    ...
    /*************************************************************************/
    if((fftwpsi = fftw_malloc(sizeof (fftw_complex )*ist.ngrid*ist.nthreads))==NULL) nerror("fftwpsi\n");
    //if((fftwpsi = fftw_malloc(sizeof (fftw_complex )*ist.ngrid))==NULL) nerror("fftwpsi\n");
    if((potl  = (double *) calloc(ist.ngrid, sizeof(double)))==NULL) nerror("potl\n");
    if((potq  = (zomplex *) calloc(ist.ngrid, sizeof(zomplex)))==NULL) nerror("potq\n");
    if((potqx  = (zomplex *) calloc(ist.ngrid, sizeof(zomplex)))==NULL) nerror("potqx\n");
    if((poth = (double *) calloc(ist.ngrid*ist.nthreads, sizeof(double)))==NULL) nerror("poth\n");
    ...
    double *psihomo = calloc(ist.totalhomo*ist.ngrid, sizeof(double));
    double *psilumo = calloc(ist.totallumo*ist.ngrid, sizeof(double));
    ...
    while (counter >= indexfirsthomo && thomo < ist.maxHoleStates) {
        ...
        counter--;
        if (counter >= 0){fseek(ppsi, -2 * foffset, SEEK_CUR);} 
    }
    ...
    //     double *ipr;
    //     printf("\nCalculating IPR...\n");
    //     if ((ipr = calloc(nstates, sizeof(double)))==NULL){
    //     printf("\nOUT OF MEMORY: IPR\n");
    //     nerror("ipr");
    //     }
    //     IPR(psi, ipr, nstates, par, ist);
    //     free(ipr);
    // }
    /*                              END CHANGES                           */
    /*************************************************************************/
    ...
    writeSeparation(stdout);
    printf("\nCalculating single-particle optical properties...\n"); //fflush(0);
    tci = (double)clock(); twi = (double)time(NULL);
    if (( mux = (double *) calloc(ist.totallumo*ist.totalhomo, sizeof(double)))==NULL) nerror("mux\n"); // <psi_i|ux|psi_a>
    if(( muy = (double *) calloc(ist.totallumo*ist.totalhomo, sizeof(double)))==NULL) nerror("mux\n"); // <psi_i|uy|psi_a>
    if(( muz = (double *) calloc(ist.totallumo*ist.totalhomo, sizeof(double)))==NULL) nerror("mux\n"); // <psi_i|uz|psi_a>
    if(( mx  = (double *) calloc(ist.totallumo*ist.totalhomo, sizeof(double)))==NULL) nerror("mux\n"); // <psi_i|mx|psi_a>
    if(( my  = (double *) calloc(ist.totallumo*ist.totalhomo, sizeof(double)))==NULL) nerror("mux\n"); // <psi_i|my|psi_a>
    if(( mz  = (double *) calloc(ist.totallumo*ist.totalhomo, sizeof(double)))==NULL) nerror("mux\n"); // <psi_i|mz|psi_a>
    if(( rs  = (double *) calloc(ist.totallumo*ist.totalhomo, sizeof(double)))==NULL) nerror("mux\n"); // <psi_a|u|psi_i>.<psi_a|m|psi_i>
    printf("\tdipole matrix elements...\n"); //fflush(0);
    dipole(vx, vy, vz, psi, mux, muy, muz, eval, ist, par);
    printf("\tmagnetic dipole matrix elements...\n"); //fflush(0);
    mag_dipole(vx, vy, vz, psi, mx, my, mz, eval, planfw, planbw, fftwpsi, ist, par);
    printf("\trotational strengths...\n"); //fflush(0);
    rotational_strength(rs, mux, muy, muz, mx, my, mz, eval, ist);
    printf("done calculating single particle optical properties, CPU time (sec) %g, wall run time (sec) %g\n",
    ((double)clock()-tci)/(double)(CLOCKS_PER_SEC), (double)time(NULL)-twi); fflush(0);
  
    if (par.nocalcExciton == 1){
        printf("\nEXITING PROGRAM: SOLVE BETHE-SALPETHER (without computing excitonic states)\n");
        return 0;
    }
    ...
    printf("done calculating coulomb matrix elements, CPU time (sec) %g, wall run time (sec) %g\n",
    ((double)clock()-tci)/(double)(CLOCKS_PER_SEC), (double)time(NULL)-twi); fflush(0);
    ...
    writeSeparation(stdout);
    printf("\nCalculating bethe-salpether coefficients...\n"); //fflush(0);
    tci = (double)clock(); twi = (double)time(NULL);
    bethe_salpeter(bsmat, h0mat, psi, vz, mux, muy, muz, mx, my, mz, ist, par);
    printf("done calculating bethe-salpether, total CPU time (sec) %g, wall run time (sec) %g\n",
    ((double)clock()-tci)/(double)(CLOCKS_PER_SEC), (double)time(NULL)-twi); fflush(0);
  
    
    printf("\nEXITING PROGRAM: SOLVE BETHE-SALPETHER\n");
    writeSeparation(stdout);
    writeCurrentTime(stdout);
